...
def build_tree_from_local_commits() -> dict[str, Branch]:
        ...
        if commit.branch_name in local_branches:
            ...
            for preceding_commit in reversed(preceding_commits):
                ...
                if branch_name in tree and branch_name != commit.branch_name:
                    ...
                    break
                elif preceding_commit.message in [c.message for c in commits]:
...
def generate_plan() -> str:
    ...
    commands = []
    for commit in reversed(get_local_commits()):
    ...
    lines = []
    for command, commit in zip(commands, reversed(get_local_commits())):

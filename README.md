...
Invoking `trunk edit` at this point will open an editor showing the current plan:
...
To instruct trunks to create the branching structure described above we use the `b` instruction coupled with an optional label:
...
With trunks, you can simply run `trunks update` to delete and recreate your local ephemeral branches based on the plan.
